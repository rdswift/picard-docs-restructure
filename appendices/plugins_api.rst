...
.. note::
   ...
   All registrations (``api.register_*``) are automatically removed when the plugin is disabled. The ``disable()`` function is only needed for custom cleanup (closing connections, stopping threads, etc.).
...
.. note::
   ...
   This is a convenience method. For the main ``enable()`` function and classes that naturally receive the API (like ``BaseAction``), use the passed ``api`` parameter directly.
...
.. note::
   ...
   Classes that are considered part of the API like ``Metadata``, ``Track``, ``Album``, etc. should be imported directly from ``picard.plugin3.api`` instead from elsewhere in Picard.
...
.. note::
   ...
   Values are stored in Qt settings format. Complex types (lists, dicts) are automatically serialized.
...
.. important::
   ...
   This is for **reading** files only. Do not write to the plugin directory:
...
.. seealso::
   ...
   Please see :doc:`plugins_translations` for complete translation system documentation.
...
.. seealso::
   ...
   Please see :doc:`plugins_translations` for plural forms and Common Locale Data Repository (CLDR) rules.
...
.. note::
   ...
   You must pass the class, not an instance. Picard makes ``self.api`` available inside the class to access the ``PluginApi`` instance of the plugin. The ``TITLE`` constant is the name displayed in the menu.
...
.. note::
   ...
   If your task doesn't involve a network request, you can omit ``request_factory``. However, if you're making a web request, you should always provide ``request_factory`` to ensure the request is properly cancelled if the album is removed.
...
.. important::
   ...
   Always call ``complete_album_task()`` in a ``finally`` block to ensure the task is marked complete even if an error occurs.
...
* **Always use the api parameter**: Don't import Picard internals directly
* **Use plugin_config for plugin settings**: Keeps your config isolated
* **Log appropriately**: Use ``debug`` for verbose, ``info`` for important events
* **Handle errors gracefully**: Wrap risky operations in try/except
* **Set priorities wisely**: Only use non-zero priorities when order matters
* **Pass api to parent classes**: You can use ``self.api`` in classes inherited from base classes meant to be subclassed and imported from ``picard.plugin3.api``, like ``BaseAction`` or ``OptionsPage``.
