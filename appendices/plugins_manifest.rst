...
Appendix B: :index:`Plugins Manifest <pair: plugins; manifest>`
=================================================================
...
.. note::
   ...
   For more complex plugins, consider adding ``long_description`` to provide detailed information about features, requirements, and usage.
...
.. list-table::
   ...
   * - ``uuid``
     - string
     - Unique plugin identifier (UUID v4)
   ...
   * - ``description``
     - string
     - Short description (one line, 1-200 chars)
...
.. list-table::
   ...
   * - ``authors``
     - array
     - Plugin author/contributor names
     - ``["John Doe", "Jane Smith"]``

   * - ``maintainers``
     - array
     - Plugin maintainer names
     - ``["John Doe"]``
   ...
   * - ``long_description``
     - string
     - Detailed description (multi-line, 1-2000 chars, Python-Markdown supported)
     - See below
...
.. list-table::
   ...
   * - ``name_i18n``
     - table
     - Translated names (locale → string)
     - See below

   * - ``description_i18n``
     - table
     - Translated short descriptions (locale → string)
     - See below

   * - ``long_description_i18n``
     - table
     - Translated long descriptions (locale → string)
     - See below
...
* **Keep** ``description`` **concise**: One clear sentence is better than cramming multiple ideas
* **Use** ``long_description`` **for details**: Features, requirements, configuration steps
* **Structure** ``long_description``: Use blank lines to separate sections
* **Use Markdown formatting**: ``long_description`` supports Markdown (bold, lists, code blocks, etc.)
* **Translate both**: Provide translations for both fields if possible
* **Avoid redundancy**: Don't repeat the short description in the long one
* **Be specific**: Mention actual features, not vague promises
* **Include requirements**: List any accounts, network access, or dependencies needed
...
* See `https://spdx.org/licenses/ <https://spdx.org/licenses/>`_ for a list of supported licenses
...
* See :doc:`Plugin Translations <plugins_translations>` for details on plugin translation system
...
.. note::
   ...
   The ``disable()`` function is optional. Only implement it if your plugin needs to clean up resources (close files, stop threads, disconnect from services, etc.). Most simple plugins don't need it.
...
See the :doc:`Plugins API <plugins_api>` documentation for a complete list of extension points:
...
See :doc:`Plugin Translations <plugins_translations>` for details.
...
* **Handle errors** - Use try/except for network and file operations
* **Be efficient** - Don't block the UI thread
* **Test thoroughly** - Test with various file types and edge cases
* **Document** - Add docstrings and comments
* **Follow conventions** - Use Picard's coding style
* **Version properly** - Use git tags for releases (e.g., ``v1.0.0``, ``v2.1.0``)
* **Declare dependencies** - Document required Python packages
...
See :doc:`Plugins Registry <plugins_registry>` for the registry submission process.
