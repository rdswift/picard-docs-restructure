...
Appendix E: :index:`Plugins Registry <pair: plugins; registry>`
==================================================================
...
.. list-table::
   ...
   * - ``id``
     - string
     - Yes
     - Short identifier for CLI/URL usage (lowercase, alphanumeric + hyphens)

   * - ``uuid``
     - string
     - Yes
     - Unique plugin identifier (UUID v4 from MANIFEST.toml)

   * - ``name``
     - string
     - Yes
     - Display name of the plugin (English)

   * - ``description``
     - string
     - Yes
     - Short description (one line, English)
   ...
   * - ``description_i18n``
     - object
     - No
     - Translated descriptions (locale → string)
   ...
   * - ``versioning_scheme``
     - string
     - No
     - Version tagging scheme: ``semver``, ``calver``, or ``regex:<pattern>``. Enables automatic version discovery. If omitted, uses explicit refs only.

   * - ``refs``
     - array
     - No
     - Git refs (branches/tags) available for this plugin (defaults to ``[{"name": "main"}]``). Each ref can have ``min_api_version`` and ``max_api_version`` fields.
   ...
   * - ``trust_level``
     - string
     - Yes
     - Trust level: ``official``, ``trusted``, or ``community``

   * - ``authors``
     - array
     - No
     - Plugin author/contributor names (from MANIFEST.toml)

   * - ``maintainers``
     - array
     - No
     - Plugin maintainer names (from MANIFEST.toml)

   * - ``added_at``
     - string
     - Yes
     - ISO 8601 timestamp when added to registry
...
.. note::
   ...
   API version constraints (``min_api_version``, ``max_api_version``) are specified per-ref within the ``refs`` array, not at the top level. This allows different branches to support different Picard versions.
...
.. list-table::
   ...
   * - ``description``
     - string
     - No
     - Human-readable description of this ref

   * - ``min_api_version``
     - string
     - No
     - Minimum Picard API version for this ref

   * - ``max_api_version``
     - string
     - No
     - Maximum Picard API version for this ref
...
.. list-table::
   ...
   * - **UUID**
     ...
     - All sources, past and future
     - None - UUID is permanent
   ...
   * - **url_regex**
     ...
     - Multiple URLs matching regex
     - Medium - can change hosting
...
* **User has old URL**: Plugin installed from ``https://github.com/olduser/old-repo``
* **Registry lookup**: Client checks registry for plugin by old URL
* **Redirect found**: Registry returns new URL ``https://github.com/neworg/plugin-repo``
* **Transparent update**: Client fetches updates from new URL automatically
* **Metadata updated**: Local metadata updated to track new URL
...
**URL redirects** - Plugin moved to different repository:
...
**UUID redirects** - Plugin was forked/replaced (rare):
...
Please see :doc:`Plugin Translations <plugins_translations>` for details on the translation system.
...
.. note::
   ...
   Trust level is per-plugin, not per-author. The same author can have plugins at different trust levels.
